...
export default function RemittanceBonusWidget() {
  ...
  return (
    <div className="rounded-2xl overflow-hidden shadow-[0_2px_8px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.08)] ring-1 ring-slate-200/60 dark:ring-slate-700/60 bg-gradient-to-br from-yellow-50 via-white to-amber-50 dark:bg-slate-800 transition-all duration-500 hover:-translate-y-0.5">
      {/* Header */}
      <div className="px-6 py-5 bg-gradient-to-br from-blue-500 to-blue-600 text-white dark:from-blue-600 dark:to-blue-700 transition-all duration-300">
        <div className="flex items-center justify-between gap-3">
          <h3 className="text-xl sm:text-2xl font-extrabold tracking-tight flex items-center gap-2.5 text-[#FFD700]">
            <span aria-hidden className="text-2xl">🎁</span>
            <span>CBE Remittance Bonus</span>
          </h3>
          <div className="text-sm font-bold px-3 py-1.5 rounded-md bg-white/20 text-white dark:bg-gray-900/40">
      ...
      <div className="p-4">
        <div className="space-y-3">
          {/* Info Banner */}
          <div className="p-4 bg-[#FFF9E6] dark:bg-[#F2C94C]/10 rounded-lg border border-[#F2C94C]/30 dark:border-[#F2C94C]/20">
            <p className="text-sm text-[#B8860B] dark:text-[#F2C94C] leading-relaxed font-semibold">
          ...
          {/* Rates with Bonus */}
          <div>
            <h4 className="mb-3 text-base font-bold text-[#D4AF37] dark:text-[#FFD700] tracking-wide">
              Official Rate + Bonus
            </h4>
            {isLoading ? (
                ...
                {official?.rates?.map((r: any) => (
                  <li key={r.code} className="flex items-center justify-between py-3">
                    <div className="flex items-center gap-2.5">
                      <span className={clsx('text-base font-semibold', r.code === 'USD' && 'font-bold text-lg')}>{r.code}</span>
                      {r.code === 'USD' && (
                    ...
                    <div className="flex items-center gap-2">
                      <div className="text-right">
                        <div className="text-sm text-gray-500 dark:text-gray-400 line-through font-medium">
                          {fmt(getDisplayRate(r))}
                        </div>
                        <div className="text-base font-extrabold tabular-nums text-[#D4AF37] dark:text-[#FFD700]">
          ...
          <div className="pt-3 border-t border-gray-200 dark:border-gray-800">
            <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2.5">Available through:</p>
            <div className="flex flex-wrap gap-2">
              {['EthioDirect', 'CashGo', 'FastPay', 'Western Union'].map((partner) => (
                <span
                  key={partner}
                  className="text-sm font-medium px-3 py-1.5 rounded-md bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300"
                >
                  {partner}
                </span>
              ))}
          ...
          {!isLoading && (
            <div className="pt-3 space-y-2.5">
              ...
              <p className="text-sm font-semibold text-amber-800 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 p-3 rounded-md border border-amber-200 dark:border-amber-800">
